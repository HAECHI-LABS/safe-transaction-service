...
class TransactionService:
    ...
    def get_all_txs_from_identifiers(
        ...
        logger.debug("Getting %d txs from identifiers", len(ids_to_search))
        ids_with_cached_txs = {
        ...
        logger.debug("Got %d cached txs from identifiers", len(ids_with_cached_txs))
        ids_not_cached = [
            hash_to_search
            for hash_to_search in ids_to_search
            if hash_to_search not in ids_with_cached_txs
        ]
        logger.debug("%d not cached txs from identifiers", len(ids_not_cached))
        ...
        logger.debug("Got %d Multisig txs from identifiers", len(ids_with_multisig_txs))
        ...
        logger.debug("Got %d Module txs from identifiers", len(ids_with_module_txs))
        ...
        logger.debug(
            "Got %d Plain Ethereum txs from identifiers",
        ...
        logger.debug("Got %d Transfers from identifiers", len(transfers))
        ...
        logger.debug("Got %d tokens for transfers from tokens database", len(tokens))
